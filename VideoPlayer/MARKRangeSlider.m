...
- (void)setUpViewComponents
{
    ...
    // Add PopOver
    _resourceBundle = [[[NSBundle bundleForClass:self.class] resourcePath]
                       stringByAppendingPathComponent:@"RETrimControl.bundle"];;
    
    _leftPopover = [[RETrimPopover alloc] initWithFrame:CGRectMake(-9, -28, 10, 50) resourceBundle:_resourceBundle :0];
    
    _rightPopover = [[RETrimPopover alloc] initWithFrame:CGRectMake(-9, -28, 10, 50) resourceBundle:_resourceBundle :1];
...
- (void)handleLeftPan:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan || gesture.state == UIGestureRecognizerStateChanged) {
        ...
        frame.origin.x = _leftThumbImageView.frame.origin.x - 9;
        _leftPopover.frame = frame;
        
        // Change left value
        self.leftValue += translation.x / width * trackRange;
        
        _leftPopover.timeLabel.text = [NSString stringWithFormat:@"00:%02d",(int)self.leftValue];
...
- (void)handleRightPan:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan || gesture.state == UIGestureRecognizerStateChanged) {
        ...
        frame.origin.x = _rightThumbImageView.frame.origin.x - 9;
        _rightPopover.frame = frame;
        
        // Change right value
        self.rightValue += translation.x / width * trackRange;
        if(self.rightValue < 59.5){
            _rightPopover.timeLabel.text = [NSString stringWithFormat:@"00:%02d",(int)self.rightValue];
        }else
            _rightPopover.timeLabel.text = [NSString stringWithFormat:@"01:00"];
...
-(void)RemovePopOver{
    [_leftPopover removeFromSuperview];
    [_rightPopover removeFromSuperview];
}
